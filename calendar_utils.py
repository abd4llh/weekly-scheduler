...
def render_calendar_html(events, week_start, start_hour=6, end_hour=23, px_per_hour=72):
    ...
    hours = "".join(
        f'<div class="hour-line" style="top:{(h-start_hour)*px_per_hour}px"></div><div class="hour-label" style="top:{max((h-start_hour)*px_per_hour-8,0)}px">{h:02d}:00</div>'
        for h in range(start_hour, end_hour + 1)
    )
    heads = "".join(
        f'<div class="day-head"><div class="dow">{DAY_SHORT[i]}</div><div class="date-num">{(week_start+timedelta(days=i)).day}</div></div>'
        for i in range(7)
    )
...
def events_to_ics(events, week_start, calendar_name="Weekly Scheduler"):
    ...
    for e in events:
        ...
        stt = datetime.combine(sd, time(e.start_min // 60, e.start_min % 60))
        enn = datetime.combine(sd, time(e.end_min // 60, e.end_min % 60))
