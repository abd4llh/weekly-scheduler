...
from .generic_solver import WeeklyOptimizer
...
def _optimizer_event_to_legacy(event, week_start: datetime, task: Task) -> Event:
    ...
    if event.source.value == "fixed_task":
        ...
        explanation = "Placed using the configured routine window and sequence."
    else:
        explanation = "Placed from structured task constraints and preferences."
    return Event(
        title=event.title,
        day_index=day_index,
        start_min=start_min,
...
def optimize_legacy_week(tasks: Sequence[Task], week_start: date | datetime, settings: Dict) -> Tuple[List[Task], List[Event], List[UnscheduledTask], List[Dict], Dict]:
    ...
    request = legacy_tasks_to_plan_request(
        ...
        preferred_daily_flexible_min=int(settings.get("preferred_daily_flexible_min", DEFAULT_DAILY_TARGET_MIN)),
        max_daily_flexible_min=int(settings.get("max_daily_flexible_min", DEFAULT_DAILY_MAX_MIN)),
    ...
    result = WeeklyOptimizer(OptimizerConfig(max_solve_seconds=25.0, num_search_workers=8, random_seed=7)).solve(request)
    canonical_by_id = {task.id: task for task in request.tasks}
    legacy_by_title = _legacy_task_lookup(normalized_tasks)
    events, unscheduled, issues = [], [], []
    if result.status in {SolverStatus.OPTIMAL, SolverStatus.FEASIBLE}:
        for optimized_event in result.events:
            ...
            events.append(_optimizer_event_to_legacy(optimized_event, start_dt, legacy_task))
    else:
        reason = str(result.diagnostics.get("reason") or result.diagnostics.get("status_name") or "The optimizer could not find a feasible schedule.")
        issues.append({"level": "error", "task": "Optimization model", "message": reason})
