...
class Scheduler:
    ...
    def add_unscheduled(self, t: Task, reason: str):
        if not any(u.title == t.title and u.reason == reason for u in self.unscheduled):
            self.unscheduled.append(UnscheduledTask(t.title, reason, t.task_type, t.priority, int(t.duration_min), t.notes, t.category))
    ...
    def fits_load(self, d, dur, t: Task):
        if t.task_type not in ["Flexible", "Multi-session"]: return True
        if self.flex_load[d] + dur > self.daily_flex_cap[d]: return False
        if t.energy == "High" and self.high_load[d] + dur > self.daily_high_cap[d]: return False
        if t.priority == "Optional" and any(u.priority in ["Critical", "High"] for u in self.unscheduled): return False
        return True

    def add_event(self, e: Event, allow=False, source_task: Task = None):
        if source_task and (not e.category or e.category == "Other"):
            e.category = source_task.category
        self.events.append(e)
        if not allow:
            self.busy[e.day_index].append((e.start_min, e.end_min, e.title)); self.busy[e.day_index].sort()
        if source_task and source_task.task_type in ["Flexible", "Multi-session"]:
            ...
            if source_task.energy == "High": self.high_load[e.day_index] += dur
    ...
    def find_slot(self, t: Task, dur: int, preferred_days=None):
        wins = self.windows(t)
        if preferred_days: wins = [x for x in wins if x[0] in preferred_days] + [x for x in wins if x[0] not in preferred_days]
        for d,a,b,why in wins:
            if not self.fits_load(d, dur, t): continue
            s=a
            while s + dur <= b:
                if self.is_free(d,s,s+dur,t.can_overlap):
                    return d,s,s+dur,f"Placed on {DAY_NAMES[d]} {minutes_to_hhmm(s)}–{minutes_to_hhmm(s+dur)} because it is {t.priority.lower()} priority and matches the {why} window. Mode: {self.planning_mode}."
                s += self.slot_min
        return None
    ...
    def add_focus_guard(self):
        ...
        for d,s,e,title in guards:
            if self.is_free(d,s,e): self.add_event(Event(title,d,s,e,"High","Focus Guard","Protect vulnerable scrolling moments.","Focus Guard block.","Focus"))

    def schedule_fixed(self, t: Task):
        from models import DAY_TO_INDEX
        d = DAY_TO_INDEX.get(str(t.fixed_day).lower()); s = hhmm_to_minutes(str(t.fixed_start))
        ...
        self.add_event(Event(t.title,d,s,e,t.priority,t.title,t.notes,f"Scheduled as a fixed event on {DAY_NAMES[d]} at {minutes_to_hhmm(s)}.",t.category), t.can_overlap)
    ...
    def schedule_recurring(self, t: Task):
        ...
        if "gym" in title: days=[1,3,5]
            ...
            for d in range(7): self.add_event(Event(t.title,d,1200,1350,t.priority,t.title,t.notes,"Protected relationship time; overlap allowed.",t.category), True)
            return
        else:
        ...
        for d in days[:t.sessions_per_week]:
            targets = [(390,510)] if "gym" in title else ([(1050,1080)] if ("german" in title and d in [1,3,5]) else ([(375,405)] if "german" in title else ([(1080,1200)] if "cooking" in title else [])))
            placed=False
            for s,e in targets:
                if self.is_free(d,s,e,t.can_overlap):
                    self.add_event(Event(t.title,d,s,e,t.priority,t.title,t.notes,"Placed by recurring routine rule.",t.category), t.can_overlap); placed=True; count += 1; break
            if not placed:
                slot = self.find_slot(t, int(t.duration_min), [d])
                if slot: self.add_event(Event(t.title,*slot[:3],t.priority,t.title,t.notes,slot[3],t.category), t.can_overlap); count += 1
        if count < min(t.sessions_per_week, len(days)): self.add_unscheduled(t, f"Only scheduled {count}/{t.sessions_per_week} recurring sessions.")
    ...
    def schedule_flex(self, t: Task):
        ...
        while remaining > 0:
            max_block = min(int(t.max_block_min), remaining); min_block = min(int(t.min_block_min), max_block); placed=False
            for block in range(max_block, min_block-1, -15):
                slot = self.find_slot(t, block)
                if slot:
                    self.add_event(Event(t.title,*slot[:3],t.priority,t.title,t.notes,slot[3],t.category), t.can_overlap, source_task=t)
                    remaining -= block; scheduled += block; placed=True; break
            if not placed: break
        if remaining > 0: self.add_unscheduled(t, f"Scheduled {scheduled}/{total} minutes; {remaining} minutes did not fit within realistic daily capacity limits.")
    ...
    def schedule(self, tasks: List[Task], include_focus_guard=False):
        ...
        rest.sort(key=lambda t:(PRIORITY_SCORE.get(t.priority,2), ENERGY_SCORE.get(t.energy,2), -1 if t.priority=="Optional" else t.duration_min), reverse=True)
