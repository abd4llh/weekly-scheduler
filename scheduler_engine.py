...
class Scheduler:
    ...
    def add_unscheduled(self, task, reason):
        self.unscheduled.append(UnscheduledTask(task.title, reason, task.task_type, task.priority, int(task.duration_min), task.notes, task.category))
    ...
    def windows(self, task):
        ...
        if task.preferred_time == "Morning":
            for d in range(7): windows.append((d, 375, 570, "morning preference"))
        elif task.preferred_time == "Workday" or task.location == "Lab":
            for d in range(5): windows += [(d,540,720,"deep-work morning"),(d,780,930,"workday afternoon"),(d,960,1050,"late follow-up")]
        elif task.preferred_time == "Evening":
            for d in range(7): windows.append((d,1080,1350,"evening preference"))
        elif task.preferred_time == "Weekend":
            for d in [5,6]: windows += [(d,420,720,"weekend morning"),(d,840,1140,"weekend afternoon")]
        else:
            for d in range(5): windows += [(d,540,720,"default morning"),(d,780,1050,"default afternoon"),(d,1080,1200,"early evening")]
            if not self.protect_weekend or task.priority in ["Critical","High"]:
                for d in [5,6]: windows += [(d,540,720,"weekend fallback"),(d,840,1020,"weekend fallback")]
    ...
    def score_slot(self, task, day, start, end, reason):
        ...
        if "morning" in reason and task.energy in ["High","Physical"]: score += 25
        if task.category in ["Lab","Writing"] and day <= 3: score += 18
        if task.category == "Admin" and end-start <= 30: score += 14
        if task.category in ["Home","Social","Optional"] and day in [5,6]: score += 12
        if task.priority == "Optional": score -= 40
        if day == 5: score -= 20 if task.energy == "High" else 5
        if day == 6: score -= 45 if task.energy == "High" else 10
        if self.planning_mode == "Social weekend mode" and day in [5,6]: score -= 35
        if self.planning_mode == "Recovery week" and task.energy == "High": score -= 25
        if self.planning_mode == "Deadline mode" and task.priority in ["Critical","High"]: score += 25
        score -= self.flex_load[day] / 20
        if task.energy == "High": score -= self.high_load[day] / 15
        if start >= 960 and task.energy == "High": score -= 18
        return score

    def find_slot(self, task, duration, preferred_days=None):
        best = None
        for day, a, b, reason in self.windows(task):
            start = a
    ...
    def add_focus_guard(self):
        ...
        for day, start, end, title in guards:
            if self.is_free(day, start, end):
                self.add_event(Event(title, day, start, end, "High", "Focus Guard", "Protect vulnerable scrolling moments.", "Focus Guard block.", "Focus"))
    ...
    def schedule_recurring(self, task):
        ...
        if "gym" in title: days = [1,3,5]
            ...
            for day in range(7):
                self.add_event(Event(task.title,day,1200,1350,task.priority,task.title,task.notes,"Protected relationship time; overlap allowed.",task.category), True)
            return
        else:
            days = list(range(7))[:task.sessions_per_week] if task.sessions_per_week not in [2,3,5] else ({2:[1,4],3:[0,2,4],5:list(range(5))}[task.sessions_per_week])
        for day in days[:task.sessions_per_week]:
            if "gym" in title: targets = [(390,510)]
            elif "german" in title: targets = [(1050,1080)] if day in [1,3,5] else [(375,405)]
            elif "cooking" in title: targets = [(1080,1200)]
            else: targets = []
            placed = False
            for start, end in targets:
                if self.is_free(day,start,end,task.can_overlap):
                    self.add_event(Event(task.title,day,start,end,task.priority,task.title,task.notes,"Placed by recurring routine rule.",task.category), task.can_overlap)
                    placed = True
                    count += 1
                    break
            if not placed:
                ...
                if slot:
                    ...
                    count += 1
        if count < min(task.sessions_per_week, len(days)):
            self.add_unscheduled(task, f"Only scheduled {count}/{task.sessions_per_week} recurring sessions because compatible routine slots were full.")

    def schedule_flex(self, task):
        if task.priority == "Optional" and any(u.priority in ["Critical","High"] for u in self.unscheduled):
            return self.add_unscheduled(task, "Skipped optional task because high-priority work is already unscheduled.")
        total = int(task.duration_min) * int(task.sessions_per_week) if ("cabinet" in task.title.lower() and task.task_type == "Multi-session") else int(task.duration_min)
        remaining = total
        scheduled = 0
        while remaining > 0:
